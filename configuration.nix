...
in
{
  ...
  environment.etc."polyflow/metadata.json" = lib.mkIf (builtins.pathExists ./metadata.json) {
    source = ./metadata.json;
    mode = "0400";  # Read-only for root
  };

  # Copy sops age key into the image if it exists
  # This allows runtime decryption of secrets
  systemd.tmpfiles.rules = lib.mkIf (builtins.pathExists ./sopskey.txt) [
    "d /var/lib/sops-nix 0755 root root -"
    "f /var/lib/sops-nix/key.txt 0400 root root - ${builtins.readFile ./sopskey.txt}"
  ];
  ...
  sops = lib.mkIf (builtins.pathExists ./metadata.json) {
    defaultSopsFile = ./metadata.json;  # Use source path for build, gets copied to /etc/polyflow/metadata.json
